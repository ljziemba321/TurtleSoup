...
int i;                  //Incrementor for tabs function.
int iCount = 0;         //How many tabs we need to print.
char indents[25];
char shellName[25];     //Name of the shell to print off when using shell.
...
%%
       ...
       InstList : InstList Inst  { sprintf($$.str, "%s%s", $1.str, $2.str); }
           ...
           Inst : INSTINCT { iCount++; tabs(); } NAME InstOrderList ENDINSTINCT  {
                                                                                  iCount--;
                                                                                  tabs(); 
                                                                                  addTab($3.str, 3); 
                                                                                  sprintf($$.str, "def %s(turtName):\n%s\n", $3.str, $4.str);
                                                                                 }
	            ;
        ...
        Declars : Declars NewTurtle  { sprintf($$.str, "%s%s", $1.str, $2.str); } 
		        ...
		        | NewVar			 { /* Nothing to pass because python handles it. */ }
    ...
    InstCommand : NAME INSTINCT NAME ';'  {
                                           if(inTab($1.str) == 1 && inTab($3.str) == 3)
                                               sprintf($$.str, "%s%s(%s)\n", indents, $3.str, $1.str);
		   							       else { 
                                               printf("Instinct/Turtle name not declared!!!\n"); 
                                               exit(1);} }
		        ;


         DoLoop : DO 	{ iCount++; tabs(); } Expr Commands ENDDO ';' {
                                                                       iCount--; 
                                                                       tabs();
    ...
    TurtCommand : NAME Order ';' {
                                  if(inTab($1.str) == 1)
                                      sprintf($$.str, "%s%s.%s", indents, $1.str, $2.str);
							      else {
                                      printf("Turtle %s NOT declared or Variable %s is of wrong type\n", $1.str, $1.str); 
                                      exit(1);} }
		        ;   
 ...
 ShellOrderList : ShellOrderList Order  { sprintf($$.str, "%s%s%s.%s", $1.str, indents, shellName, $2.str); }
  ...
  InstOrderList : InstOrderList Order  { sprintf($$.str, "%s%sturtName.%s", $1.str, indents, $2.str); }
      ...
      NewTurtle : TURTLE NAME	';'  {
                                      addTab($2.str, 1); 
                                      sprintf($$.str, "%s = turtle.Turtle()\n%s.shape(\"turtle\")\n%s.color(\"green\")\n%s.speed(1)\n", $2.str, $2.str, $2.str, $2.str);
                                     }
		        ;     
            ...
            Expr: Expr OP T		  { sprintf($$.str,"%s %s %s", $1.str, $2.str, $3.str); }
              ...
              T : '(' Expr ')'  { sprintf($$.str,"(%s)", $2.str); }
                ...
                |  NAME			{ 
                                 if(inTab($1.str) == 4)
                                     sprintf($$.str, "%s", $1.str);
					             else if(inTab($1.str) == 2) {
                                     printf("Variable %s is declared but has no value!!\n", $1.str); 
                                     exit(1);}
					             else { 
                                     printf("Variable %s is Undeclared or Incorrect type!!\n",$1.str); 
                                     exit(1);} }
                ;
